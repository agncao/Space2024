//对象实时更新器
let EntityUpdater_Magicdraw = (function() {
	//差值方法
    var interpolators = {
    ...
    var EntityUpdater_Magicdraw = {
        ...
        update:function(jd,entity,value){
        	...
        	//更新除了位置和姿态的数据
        	EntityUpdater_Magicdraw.commonUpdater(jd,entity,value);
        	
        	//更新位置和姿态，更新方法根据entityType从EntityUpdater_Magicdraw.updaters来获取
			var positionOrienUpdater = EntityUpdater_Magicdraw.updaters[entity.entityType.toLowerCase()];
                ...
                if (arti) {
                    ...
                    for (const key in arti) {
                        ...
                        if (entity.model.articulations.hasProperty(key)) {
                            ...
                            property.forwardExtrapolationDuration = 1000;
        ...
        createEntity: function(jd, key, value,globalAttribute) {
            ...
            if(value.hasOwnProperty("CustomProperty") && value.CustomProperty){
            	console.log(value.CustomProperty);
            	
            	if(value.CustomProperty.IsHoldPosition){
            		data.A805_Properties.realTime.lookAheadPropagator ="HoldPosition";
            	}
            }
            data.A805_Properties.realTime.lookAhead =10;
            		data.A805_Properties.realTime.lookBehind =10;
            data.model.heightReference = undefined;
            data.billboard.heightReference = undefined;
            data.path.clampToGround = false;
            data.polyline.clampToGround = false;
            data.label.heightReference = undefined;
            console.log(data);
            //根据data配置，创建并返回新建的Entity对象
            return Cesium.CzmlDataSource.processEntityPacket(data);
        }
    };
    return EntityUpdater_Magicdraw;
})();